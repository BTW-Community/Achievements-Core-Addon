...
public class FCBlockFurnaceBrick extends FCBlockFurnace
{
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float xOffset, float yOffset, float zOffset)
    {
        ...
        else
        {
            ...
            FCTileEntityFurnaceBrick entityFurnace = (FCTileEntityFurnaceBrick)world.getBlockTileEntity(x, y, z);
            ItemStack cookStack = entityFurnace.GetCookStack();

            if (yOffset > 0.375F)
            {
                if (cookStack != null)
                {
                    entityFurnace.GivePlayerCookStack(player, side);
                    EventDispatcher.onCooked(player, cookStack);
                    return true;
                }

                if (equippedStack != null && this.IsValidCookItem(equippedStack))
                {
                    if (!world.isRemote)
                    {
                        entityFurnace.AddCookStack(new ItemStack(equippedStack.itemID, 1, equippedStack.getItemDamage()));
                    }
            ...
            else if (yOffset < 0.375F && equippedStack != null)
            {
                Item equippedItem = equippedStack.getItem();
                int equippedDamage = equippedStack.getItemDamage();

                if (equippedItem.GetCanBeFedDirectlyIntoBrickOven(equippedDamage))
                {
                    if (!world.isRemote)
                    {
                        int var17 = entityFurnace.AttemptToAddFuel(equippedStack);
