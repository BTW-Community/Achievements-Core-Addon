...
public class FCBlockCampfire extends BlockContainer
{
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float xOffset, float yOffset, float zOffset)
    {
        ...
        if (equippedStack != null)
        {
            ...
            FCTileEntityCampfire entityCampfire;

            if (!this.GetHasSpit(world, x, y, z))
            {
                if (equippedItem == FCBetterThanWolves.fcItemChiselWood)
                {
                    this.SetHasSpit(world, x, y, z, true);
                    entityCampfire = (FCTileEntityCampfire)world.getBlockTileEntity(x, y, z);
                    entityCampfire.SetSpitStack(equippedStack);
            ...
            {
                entityCampfire = (FCTileEntityCampfire)world.getBlockTileEntity(x, y, z);
                itemStack = entityCampfire.GetCookStack();

                if (itemStack == null)
                {
                    if (this.IsValidCookItem(equippedStack))
                    {
                        ItemStack spitItemstack = entityCampfire.GetSpitStack();

                        if (spitItemstack.getItemDamage() == 0)
                        {
                            entityCampfire.SetCookStack(equippedStack);
                        }
                        else
                        {
                            entityCampfire.SetSpitStack((ItemStack)null);
                            this.SetHasSpit(world, x, y, z, false);

                            if (!world.isRemote)
                            {
                                ItemStack var15 = equippedStack.copy();
                                var15.stackSize = 1;
                                FCUtilsItem.EjectStackWithRandomOffset(world, x, y, z, var15);
                                FCUtilsItem.EjectSingleItemWithRandomOffset(world, x, y, z, FCBetterThanWolves.fcItemSawDust.itemID, 0);
                                world.playAuxSFX(2271, x, y, z, 0);
                            }
                        }
                ...
                else if (itemStack.itemID == equippedStack.itemID && equippedStack.stackSize < equippedStack.getMaxStackSize())
                {
                    player.worldObj.playSoundAtEntity(player, "random.pop", 0.2F, ((player.rand.nextFloat() - player.rand.nextFloat()) * 0.7F + 1.0F) * 2.0F);
                    ++equippedStack.stackSize;
                    entityCampfire.SetCookStack((ItemStack)null);
                    EventDispatcher.onCooked(player, itemStack);
                    return true;
                }
            }

            if (this.m_iFireLevel > 0 || this.GetFuelState(world, x, y, z) == 2)
            {
                int var17 = equippedStack.getItemDamage();

                if (equippedItem.GetCanBeFedDirectlyIntoCampfire(var17))
                {
                    if (!world.isRemote)
                    {
                        FCTileEntityCampfire var19 = (FCTileEntityCampfire)world.getBlockTileEntity(x, y, z);
                        world.playSoundEffect((double)x + 0.5D, (double)y + 0.5D, (double)z + 0.5D, "mob.ghast.fireball", 0.2F + world.rand.nextFloat() * 0.1F, world.rand.nextFloat() * 0.25F + 1.25F);
                        var19.AddBurnTime(equippedItem.GetCampfireBurnTime(var17));
                    }
        ...
        {
            FCTileEntityCampfire entityCampfire = (FCTileEntityCampfire)world.getBlockTileEntity(x, y, z);
            ItemStack cookItemstack = entityCampfire.GetCookStack();

            if (cookItemstack != null)
            {
                FCUtilsItem.GivePlayerStackOrEject(player, cookItemstack, x, y, z);
                entityCampfire.SetCookStack((ItemStack)null);
                EventDispatcher.onCooked(player, cookItemstack);
                return true;
            }

            itemStack = entityCampfire.GetSpitStack();

            if (itemStack != null)
            {
                FCUtilsItem.GivePlayerStackOrEject(player, itemStack, x, y, z);
                entityCampfire.SetSpitStack((ItemStack)null);
                this.SetHasSpit(world, x, y, z, false);
                return true;
            }
